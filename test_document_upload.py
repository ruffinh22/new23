...
try:
    ...
    print(f"✅ Utilisateur trouvé: {agent.matricule} ({agent.first_name} {agent.last_name})")
    print(f"   Rôle: {agent.role}")
    print(f"   Pôle: {agent.pole.name if agent.pole else '❌ NON ASSIGNÉ'} (ID: {agent.pole.id if agent.pole else 'N/A'})")
    print(f"   Filiale (Branch): {agent.branch.name if agent.branch else '❌ NON ASSIGNÉ'} (ID: {agent.branch.id if agent.branch else 'N/A'})")
    print(f"   Service (Department): {agent.department.name if agent.department else 'Optionnel'} (ID: {agent.department.id if agent.department else 'N/A'})")
...
print(f"✅ Tokens obtenu via l'endpoint")
...
try:
    ...
    print(f"   - pole_id: {access_payload.get('pole_id')} ✅" if access_payload.get('pole_id') else f"   - pole_id: ❌ MANQUANT")
    print(f"   - branch_id: {access_payload.get('branch_id')} ✅" if access_payload.get('branch_id') else f"   - branch_id: ❌ MANQUANT")
...
try:
    ...
    print(f"✅ Document trouvé dans la base de données")
    ...
    print(f"\n📁 Chemin du dossier:")
    print(f"   Dossier assigné: {document.folder.name if document.folder else '❌ PAS DE DOSSIER'}")
    
    if document.folder:
        folder_path = []
        current = document.folder
        while current:
            folder_path.insert(0, f"{current.name} (ID: {current.id}, Type: {current.folder_type})")
        ...
        while current:
            if current.id == expected_branch.id:
                found_branch = True
                print(f"   ✅ Le document EST dans la bonne filiale: {expected_branch.name}")
                break
            current = current.parent
        
        if not found_branch:
            print(f"   ❌ ERREUR: Le document n'est PAS dans la filiale {expected_branch.name}")
        ...
        if parent and parent.folder_type not in ['filiale', 'pole']:
            print(f"   ✅ Structure Année/Mois correcte")
        
    else:
        print(f"   ❌ ERREUR: Document sans dossier!")
...
print(f"Vérifiez que le dossier contient le document dans l'admin Django.")
