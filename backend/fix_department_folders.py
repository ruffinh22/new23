...
def fix_department_folders():
    ...
    all_departments = Department.objects.select_related(
        "branch", "branch__folder", "folder"
    ).all()
    ...
    for dept in all_departments:
        ...
        if actual_parent != expected_parent:
            ...
            print(
                f"   Parent actuel: {actual_parent.name if actual_parent else 'NULL (RACINE)'}"
            )
            print(
                f"   Parent attendu: {expected_parent.name if expected_parent else 'NULL'}"
            )
