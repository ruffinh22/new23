...
try:
    ...
    if response.status_code == 200:
        ...
        print(
            f"  Count: {len(data) if isinstance(data, list) else data.get('count', 'N/A')}"
        )
...
try:
    response = requests.get(
        f"{BASE_URL}/api/folders/?folder_type=pole", headers=HEADERS
    )
    if response.status_code == 200:
        data = response.json()
...
try:
    response = requests.get(
        f"{BASE_URL}/api/folders/?folder_type=filiale", headers=HEADERS
    )
    if response.status_code == 200:
        data = response.json()
...
try:
    response = requests.get(
        f"{BASE_URL}/api/folders/?folder_type=service", headers=HEADERS
    )
    if response.status_code == 200:
        data = response.json()
...
try:
    ...
    if filiale:
        response = requests.get(
            f"{BASE_URL}/api/folders/{filiale.id}/children/", headers=HEADERS
        )
        if response.status_code == 200:
            data = response.json()
            children_count = (
...
try:
    ...
    if service:
        response = requests.get(
            f"{BASE_URL}/api/folders/{service.id}/ancestors/", headers=HEADERS
        )
        if response.status_code == 200:
            data = response.json()
            ancestors_count = (
            ...
            if isinstance(data, list):
                for ancestor in data:
                    print(
                        f"    - {ancestor.get('name')} ({ancestor.get('folder_type')})"
                    )
        else:
            print(f"  Status: {response.status_code} ❌")
except Exception as e:
