...
def test_notification_system():
    ...
    test_file = SimpleUploadedFile(
        "test_doc.txt", b"Contenu du document de test", content_type="text/plain"
    )
    ...
    validation_result = DocumentValidationResult.objects.create(
        document=doc, status="PASSED", errors=[], warnings=[]
    )
    ...
    if admin1_notifs.exists() and admin2_notifs.exists() and agent_notifs.exists():
        ...
        print(
            f"    Admin1: {admin1_notifs.count()}, Admin2: {admin2_notifs.count()}, Agent: {agent_notifs.count()}"
        )
    ...
    test_file2 = SimpleUploadedFile(
        "test_doc2.txt", b"Contenu du document de test 2", content_type="text/plain"
    )
    ...
    if admin2_notifs.exists() and admin1_notifs.exists() and agent_notifs.exists():
        ...
        print(
            f"    Admin2: {admin2_notifs.count()}, Admin1: {admin1_notifs.count()}, Agent: {agent_notifs.count()}"
        )
    ...
    for type_obj in types:
        count = Notification.objects.filter(
            notification_type=type_obj["notification_type"]
        ).count()
