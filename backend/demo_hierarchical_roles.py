...
filiale_cameroun = Folder.objects.get(name="Cameroun", folder_type="filiale", parent=pole_com)
...
filiale_manager = User.objects.filter(role='FILIALE_MANAGER', branch=filiale_benin).first()
...
service_manager = User.objects.filter(role='SERVICE_MANAGER', department=service_benin).first()
...
for user in users:
    ...
    print(f"\n   🔑 Accès:")
    print(f"      - Pôle Commercial:            {'✓' if has_access_pole_com_all else '✗'}")
    print(f"      - Filiale Bénin:              {'✓' if has_access_fil_benin else '✗'}")
    print(f"      - Filiale Cameroun:           {'✓' if has_access_fil_cameroun else '✗'}")
