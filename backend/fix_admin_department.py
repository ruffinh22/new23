...
try:
    ...
    print(f"  Department Display: {admin_user.get_department_display() if admin_user.department else 'None'}")
    
    # Set department to None if not already
    if admin_user.department:
        admin_user.department = None
        admin_user.save()
        print(f"✓ Set department to NULL for ADMIN001")
    ...
    print(f"✓ Created ADMIN001 with department=None")
