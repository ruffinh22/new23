...
poles = list(Folder.objects.filter(parent__isnull=True, folder_type='pole').order_by('name'))
...
try:
    ...
    if not admin_user:
        print(Fore.YELLOW + "⚠️  Aucun superuser trouvé, les règles seront sans créateur" + Style.RESET_ALL)
        admin_user = None
except Exception as e:
    print(Fore.YELLOW + f"⚠️  Erreur lors de la recherche du superuser: {e}" + Style.RESET_ALL)
    admin_user = None
...
for idx, rule_template in enumerate(example_rules, 1):
    ...
    try:
        ...
        if not rh_pole:
            ...
            destination = Folder.objects.filter(
                folder_type='service'
            ).first()
        else:
