...
from apps.users.models import Branch, User, Department
...
for idx, rule in enumerate(routing_rules, 1):
    ...
    if rule.branch:
        ...
        print(f"   Branche: ✅ GLOBALE (toutes les filiales)")
...
for rule in routing_rules:
    ...
    if rule.branch:
        ...
        if rule.branch.parent is None or rule.branch.parent.folder_type != 'pole':
            rule_issues.append(
                f"⚠️ Branche '{rule.branch.name}' n'a pas de Pôle parent"
            )
    ...
    for field, condition in rule.conditions.items():
        if field not in ['branch', 'department', 'document_type', 'matricule_prefix']:
            warnings.append(
                f"⚠️ [{rule.name}] Condition inconnue: {field}"
            )
...
branch_usage = RoutingRule.objects.values(
    'branch__name',
    'branch__id',
    'branch__folder_type'
...
if not list(branch_usage):
    ...
    for usage in branch_usage:
        ...
        count = usage['count']
        print(f"   • {branch_name} (ID:{branch_id}, type:{branch_type}): {count} règles")
...
if total_rules == 0:
    recommendations.append("➕ Créer des règles de routage pour automatiser le flux de documents")
elif compatibility_issues:
    recommendations.append("🔧 Corriger les règles avec problèmes de compatibilité identifiés ci-dessus")
    recommendations.append("   Utiliser: python manage.py update_routing_rules_for_hierarchy")

# Vérifier hiérarchie
if poles != 8 or filiales != 56 or services != 56:
    recommendations.append("🔄 Recréer la hiérarchie (les nombres ne correspondent pas)")
    recommendations.append("   Utiliser: python manage.py load_poles && load_filiales && load_services")
else:
...
pole_assignments = RoutingRule.objects.filter(
    branch__folder_type='pole'
).count()
if pole_assignments > 0:
    recommendations.append(
        f"⚠️ {pole_assignments} règles pointent vers des Pôles (niveau 0) au lieu de Filiales"
    )
    recommendations.append("   Corriger: Les règles doivent pointer vers Filiales (folder_type='filiale')")
