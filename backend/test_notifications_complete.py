...
def test_approve_notification(document, admin1, agent):
    ...
    admin2_notifs = Notification.objects.filter(
        recipient__matricule="TEST_ADMIN_002", document=document
    )
    agent_notifs = Notification.objects.filter(recipient=agent, document=document)
...
def test_open_notification(document, agent, admin1):
    ...
    Notification.objects.filter(
        document=document, notification_type="DOCUMENT_OPENED"
    ).delete()
    ...
    agent_notifs = Notification.objects.filter(
        recipient=agent, document=document, notification_type="DOCUMENT_OPENED"
    )
    ...
    assert agent_notifs.count() == 1, (
        "L'agent doit recevoir 1 notification DOCUMENT_OPENED"
    )
...
def print_summary():
    ...
    print(
        "  2. APPROVE: Admin qui approuve notifié + Autres admins notifiés + Agent notifié"
    )
    print(
        "  3. REJECT: Admin qui rejette notifié + Autres admins notifiés + Agent notifié"
    )
    print("  4. OPEN: Agent notifié quand un admin ouvre son document")
    print(
        "  5. VALIDATION: Agent notifié du résultat de validation + Admins si validation réussie"
    )
