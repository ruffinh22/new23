...
rules_to_create = [
    {
        ...
        "routing_path": {"include_filiale": True, "include_document_type": True},
        "priority": 50,
    },
    {
...
for rule_config in rules_to_create:
    try:
        ...
        if not destination:
            print(
                Fore.RED + "   ❌ Pas de dossier destination trouvé" + Style.RESET_ALL
            )
            continue
...
class MockAgent:
    ...
    def __init__(self, branch_name, department_name, matricule="TEST001"):
        ...
        self.branch = Folder.objects.filter(
            name__icontains=branch_name, folder_type="filiale"
        ).first()

        # Récupérer ou créer le département
        if self.branch:
            self.department = self.branch.children.filter(
                name__icontains=department_name, folder_type="service"
            ).first()
        else:
    ...
    for rule in created_rules:
        if rule.matches(doc):
            ...
            if destination:
                ...
                print("   ⚠️  Destination non trouvée")

            break
    else:
        print("   ⚠️  Aucune règle ne correspond")
...
print("   ℹ️  Utiliser cette requête pour les nettoyer:")
print(
    f"   {Fore.CYAN}RoutingRule.objects.filter(name__startswith='TEST ').delete(){Style.RESET_ALL}"
)
