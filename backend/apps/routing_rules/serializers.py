...
class DepartmentDocumentTypeSerializer(serializers.ModelSerializer):
    ...
    target_folder = serializers.CharField(
        source="target_folder.name", read_only=True, allow_null=True
    )
    target_folder_id = serializers.IntegerField(
        source="target_folder.id", read_only=True, allow_null=True
    )
    file_type_config_name = serializers.CharField(
        source="file_type_configuration.display_name", read_only=True, allow_null=True
    )
    file_type_config_id = serializers.IntegerField(
        source="file_type_configuration.id", read_only=True, allow_null=True
    )
...
class RoutingRuleSerializer(serializers.ModelSerializer):
    ...
    destination_folder_name = serializers.CharField(
        source="destination_folder.name", read_only=True
    )
...
class RoutingRuleCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        logger.info("[RoutingRuleCreateSerializer.validate] Input data:")
        ...
        if "conditions" in data and data["conditions"]:
            ...
            logger.info(
                f"  - Conditions keys: {conditions.keys() if isinstance(conditions, dict) else 'N/A'}"
            )
