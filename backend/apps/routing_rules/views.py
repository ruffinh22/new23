...
class DepartmentDocumentTypeViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = DepartmentDocumentType.objects.all().order_by('department', 'document_type')
    ...
    def departments_list(self, request):
        ...
        departments = DepartmentDocumentType.objects.values_list('department').distinct().order_by('department')
        
        # Créer la liste avec les labels
        department_list = [
            {
                'value': dept[0],
                'label': DepartmentDocumentType.objects.filter(department=dept[0]).first().get_department_display()
            }
...
class RoutingRuleViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        logger.info(f"[RoutingRuleViewSet.perform_create] Received data:")
        ...
        logger.info(f"[RoutingRuleViewSet.perform_create] Rule saved:")
        ...
        logger.info(f"[RoutingRuleViewSet.perform_create] Re-fetched from DB:")
        logger.info(f"  - Conditions: {fresh_rule.conditions}")
        
        # Serialize and check output
        output_serializer = self.get_serializer(rule)
        logger.info(f"[RoutingRuleViewSet.perform_create] Serialized output:")
        logger.info(f"  - Data: {output_serializer.data}")
        logger.info(f"  - Conditions in output: {output_serializer.data.get('conditions')}")
    
    def create(self, request, *args, **kwargs):
        """Override create to return full RoutingRuleSerializer response."""
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"[RoutingRuleViewSet.create] Starting creation")
        ...
        logger.info(f"[RoutingRuleViewSet.create] Returning response with:")
