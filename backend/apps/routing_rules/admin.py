...
class DepartmentDocumentTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Configuration', {
            'fields': ('department', 'document_type', 'is_available')
        }),
        ('Détails', {
            'fields': ('description',)
        }),
...
class RoutingRuleAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'destination_folder', 'priority', 'is_active', 'times_applied')
    list_filter = ('is_active', 'created_at')
    search_fields = ('name', 'destination_folder__name')
    readonly_fields = ('created_by', 'created_at', 'updated_at', 'times_applied', 'last_applied')
    ...
    fieldsets = (
        ...
        ('Statistiques', {
            'fields': ('times_applied', 'last_applied', 'created_by', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
