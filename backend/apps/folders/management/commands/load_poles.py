...
class Command(BaseCommand):
    ...
    POLES = [
        ...
        {"name": "Pôle RH", "code": "POL_RH", "description": "Ressources humaines"},
    ]
            ...
            with transaction.atomic():
                ...
                for pole_data in self.POLES:
                    ...
                    if created:
                        created_count += 1
                        self.say(
                            f"✓ Created: {pole['name']} ({pole_data['code']})",
                            "success",
                        )
                    else:
                        if force:
                            ...
                            updated_count += 1
                            self.say(
                                f"↻ Updated: {pole.name} ({pole_data['code']})", "info"
                            )
                        else:
                            skipped_count += 1
                            self.say(
                                f"◌ Skipped (exists): {pole.name} ({pole_data['code']})",
                                "warning",
                            )
                ...
                if total_poles == 8:
                    self.say("✓ All 8 Pôles verified successfully!", "success")
                else:
                    self.say(
                        f"⚠ Warning: Expected 8 Pôles but found {total_poles}",
                        "warning",
                    )
                ...
                for i, pole in enumerate(
                    Folder.objects.filter(folder_type="pole").order_by("name"), 1
                ):
