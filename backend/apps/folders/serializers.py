...
class FolderTreeSerializer(serializers.ModelSerializer):
    """Sérialiseur RÉCURSIF pour retourner la structure d'arborescence complète avec enfants imbriqués."""
    ...
    class Meta:
        model = Folder
        fields = ["id", "name", "folder_type", "children", "parent_name"]
        read_only_fields = ["id", "name", "folder_type", "children", "parent_name"]

    def get_children(self, obj):
        """Retourne les enfants imbriqués de manière récursive."""
...
class FolderSerializer(serializers.ModelSerializer):
    ...
    parent_type = serializers.CharField(
        source="parent.folder_type", read_only=True, allow_null=True
    )
    ...
    def get_sous_services_count(self, obj):
        ...
        if obj.folder_type == "service":
            return obj.children.filter(
                folder_type="sub_service", is_active=True
            ).count()
        return None
