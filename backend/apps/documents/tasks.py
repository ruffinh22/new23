...
def validate_document_task(document_id):
    ...
    try:
        ...
        if is_valid:
            document.status = 'EN_ATTENTE'  # En attente d'approbation admin, pas VALIDE directement
        else:
...
def send_deadline_reminders_task():
    ...
    if not pending_documents.exists():
        return f"No pending documents found"
    ...
    for agent, documents in agents_documents.items():
        try:
            ...
            context = {
                ...
                'documents': documents,
                'count': len(documents),
                'site_url': settings.SITE_URL if hasattr(settings, 'SITE_URL') else 'http://localhost:5173',
            }
            
            # Render email template
            html_message = render_to_string(
                'documents/email_deadline_reminder.html',
                context
            )
            ...
            failed_count += 1
            log_error(f'Failed to send email to {agent.email}', exception=e, logger_instance=base_logger)
