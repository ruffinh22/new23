...
class DocumentValidator:
    ...
    def _validate_ods(self):
        ...
        try:
            ...
            self.warnings.append("Format ODS: Support limité, vérifiez la compatibilité Excel.")
        except Exception as e:
            self.errors.append(f"Erreur lors de la vérification du fichier ODS: {str(e)}")
    ...
    def _validate_csv(self):
        ...
        try:
            ...
            # Vérifier le nombre de lignes
            if self.specification.max_rows and len(rows) - 1 > self.specification.max_rows:
                self.errors.append(
...
class ExcelAdvancedValidator:
    ...
    def _validate_xlsb_advanced(self) -> Tuple[bool, List[str], List[str], Dict]:
        ...
        try:
            ...
            with zipfile.ZipFile(self.file, 'r') as zf:
                ...
                self.details['format_type'] = 'Excel XLSB (Binaire)'
                self.details['file_size'] = len(self.file.getvalue()) if hasattr(self.file, 'getvalue') else self.file.size
            ...
            self.errors.append("Impossible de valider le format XLSB (zipfile non disponible).")
        except Exception as e:
            self.errors.append(f"Erreur lors de la validation du fichier XLSB: {str(e)}")
    ...
    def _check_headers(self):
        ...
        if not headers:
            self.errors.append("La première ligne (en-têtes) ne contient aucune valeur.")
            return
    ...
    def _check_data_rows(self):
        ...
        if row_count == 0:
            self.warnings.append("Le fichier n'a pas de lignes de données (hormis l'en-tête).")
        ...
        for row in self.worksheet.iter_rows():
            for cell in row:
                ...
                if cell.value and isinstance(cell.value, str) and cell.value.startswith('#'):
                    formula_errors.append(f"Erreur de formule à {cell.coordinate}: {cell.value}")
...
class ValidationService:
    ...
    @staticmethod
    def validate_document(document_file: UploadedFile, specification) -> Tuple[bool, Dict]:
        """
