...
from apps.documents.validators import ExcelAdvancedValidator
from apps.documents.checkers import (
    DocumentValidationChecker,
    FileChecker,
)
...
def create_test_excel_file(filename, include_errors=False):
    ...
    return SimpleUploadedFile(
        filename, output.read(), content_type="application/vnd.ms-excel"
    )
...
def test_5_document_checker():
    ...
    for check_name, check_info in check_result["checks"].items():
        status = (
            ...
            if check_info["status"] == "FAILED"
            else "⚠"
        )
        print(
            f"  {status} {check_name}: {check_info['status']} - {check_info['message']}"
        )
