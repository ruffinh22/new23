...
class DocumentService:
    ...
    def organize_with_hierarchy(
        ...
        ❌ EXCEPTION: Si c'est un dossier 'received_user', on retourne le dossier directement
                      sans créer de sous-dossiers de type document.
        ...
        # 🔴 Si c'est un dossier "Received" d'utilisateur, on retourne directement
        # Ce dossier est personnel et on ne doit pas créer de sous-hiérarchie
        if is_recipient_upload and folder.folder_type == "received_user":
            import logging

            logger = logging.getLogger(__name__)
            logger.info(
                f"✅ Dossier Received personnel détecté - retour direct: {folder.name} (ID: {folder.id})"
            )
            return folder
            ...
            if folder_pole_id != agent.pole_id:
                ...
                try:
                    ...
                    correct_filiale = Folder.objects.get(
                        name=folder.name, parent=pole_folder, folder_type="filiale"
                    )
                    folder = correct_filiale
                    logger.info(
                        f"✅ Filiale corrigée: {folder.name} (ID:{folder.id}) du Pôle {pole_folder.name}"
                    )
                except Folder.DoesNotExist:
                    logger.error(
                        f"❌ Impossible de trouver la filiale {folder.name} du Pôle {agent.pole.name}"
                    )
        ...
        if (
            ...
            and not is_recipient_upload
            and folder.folder_type in ["filiale", "pole"]
        ):
            service_name = agent.department.name
            # ✅ Chercher le dossier service (peu importe son type actuuel)
            try:
                service_folder = Folder.objects.get(
                    name=service_name, parent=current_folder
                )
        ...
        if document_type:
            ...
            type_label = (
                ...
                if hasattr(document_type, "display_name")
        ...
        if folder_id and not folder:
            try:
                ...
                if agent and agent.pole and not is_recipient_upload:
                    ...
                    folder_pole_id = (
                        candidate_folder.parent_id if candidate_folder.parent else None
                    )
...
class DocumentFilterService:
    ...
    def get_accessible_documents(self):
        ...
        - User → ses propres documents + documents reçus dans ses dossiers Received
        ...
        if self.is_admin:
            ...
            # L'utilisateur voit:
            # 1. Ses propres documents (agent=user)
            # 2. Tous les documents dans ses dossiers Received
            received_folders = Folder.objects.filter(
                folder_type="received_user", owner=self.user
            ).values_list("id", flat=True)

            queryset = Document.objects.filter(
                Q(agent=self.user) | Q(folder_id__in=received_folders)
            )
