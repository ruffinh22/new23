...
class DocumentSpecificationViewSet(viewsets.ModelViewSet):
    ...
    def by_type(self, request):
        ...
        spec = get_object_or_404(
            DocumentSpecification, document_type=doc_type, is_active=True
        )
...
class DocumentViewSet(PermissionMixin, FilterMixin, viewsets.ModelViewSet):
    ...
    def received_documents(self, request):
        ...
        received_folders = Folder.objects.filter(
            owner=request.user, folder_type="received_user"
        )
        ...
        return Response(
            {
                ...
                if is_valid
                else "Le fichier contient des erreurs",
            },
    ...
    def validate(self, request, pk=None):
        ...
        is_valid, validation_result = DocumentService.validate_existing_document(
            document
        )
        ...
        return Response(
            {
                ...
                "validation_result": DocumentValidationResultSerializer(
                    validation_result
                ).data
    ...
    def check_before_send(self, request, pk=None):
        ...
        can_send, check_results = DocumentValidationChecker.full_check_before_send(
            document
        )
        ...
        return Response(
            {
                ...
                if can_send
                else "Le document ne peut pas être envoyé",
            }
        )
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        # Si le status est fourni et c'est REJETE
            ...
            # Utiliser le service de notifications
            NotificationService.notify_on_document_rejected(
    ...
    def approve(self, request, pk=None):
        ...
        if (
            not document.validation_result
            or document.validation_result.status != "PASSED"
        ):
            return Response(
    ...
    def pending_validation(self, request):
        ...
        queryset = Document.objects.filter(status="VALIDATION_EN_COURS").select_related(
            "agent", "specification", "validation_result"
        )
    ...
    def statistics(self, request):
        ...
        if agent_filter == "me":
            ...
            print(
                f"[STATISTICS] Using default queryset for user: {request.user.matricule}"
            )
            queryset = self.get_queryset()
        ...
        validation_stats = {
            "PASSED": DocumentValidationResult.objects.filter(status="PASSED").count(),
            "FAILED": DocumentValidationResult.objects.filter(status="FAILED").count(),
            "WARNING": DocumentValidationResult.objects.filter(
                status="WARNING"
            ).count(),
        }
        ...
        print(
            f"[STATISTICS] Returning stats - Total docs: {total_documents}, Agent filter: {agent_filter}"
        )
                ...
                try:
                    writer.writerow(
                        [
                            ...
                            doc.get("status", ""),
                            doc.get("file_format", ""),
                            round(doc.get("file_size", 0) / (1024 * 1024), 2)
                            if doc.get("file_size")
                            else 0,
                            doc.get("agent", {}).get("matricule", "")
                            if doc.get("agent")
                            else "",
                            doc.get("created_at", ""),
    ...
    def folder_structure(self, request):
        ...
        def build_folder_tree(folder, is_admin, user_department, current_user=None):
            ...
            if is_admin:
                ...
                # Si c'est un dossier "Received" (dossier personnel de réception), afficher TOUS les documents dedans
                if (
                    folder.folder_type == "received_user"
                    and folder.owner == current_user
                ):
                    documents = folder.documents.all().order_by("-created_at")
                else:
                    # Pour les agents dans les dossiers NORMAUX, afficher seulement leurs propres documents
                    documents = folder.documents.filter(agent=current_user).order_by(
                        "-created_at"
                    )
            ...
            for child_folder in children_folders:
                child_tree = build_folder_tree(
                    child_folder, is_admin, user_department, current_user
                )
                ...
                children.append(
                    {
                        ...
                        "agent_username": doc.agent.matricule
                        if doc.agent
                        else "Système",
                    }
                )
        ...
        # Pour les admins: TOUS les dossiers
        # Pour les agents: leurs dossiers personnels "Received" + autres dossiers publics
        if is_admin:
            root_folders = Folder.objects.filter(parent__isnull=True).order_by("name")
        else:
            # Agents: voir leurs dossiers "Received" + dossiers publics (folder_type != 'received_user')
            root_folders = (
                Folder.objects.filter(parent__isnull=True)
                .filter(
                    Q(folder_type="received_user", owner=user)  # Mon dossier Received
                    | ~Q(
                        folder_type="received_user"
                    )  # Les dossiers publics (pas received_user)
                )
                .order_by("name")
            )
        ...
        for folder in root_folders:
            folder_tree = build_folder_tree(folder, is_admin, user_department, user)
            # Ajouter le dossier seulement s'il contient des éléments (pour agents)
            # SAUF pour les dossiers personnels "Received" qui doivent toujours être visibles
            should_display = (
                is_admin
                or folder_tree["children"]  # Affiche si contient des éléments
                or (
                    folder.folder_type == "received_user" and folder.owner == user
                )  # TOUJOURS afficher mon dossier Received
            )
            if should_display:
    ...
    def view(self, request):
        ...
        try:
            ...
            if not os.path.isfile(full_path):
                return Response(
                    {"error": "Fichier non trouvé"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            response["Content-Disposition"] = (
                "inline"  # Affiche inline au lieu de télécharger
            )
            return response
        except Exception as e:
            import traceback
    ...
    def download(self, request, pk=None):
        ...
        try:
            ...
            if (
                ...
                NotificationService.notify_on_document_downloaded(
    ...
    def stats(self, request):
        ...
        if user.is_staff or user.is_superuser or getattr(user, "role", None) == "ADMIN":
            ...
            total_users = (
                User.objects.filter(role="AGENT").count()
                if hasattr(User, "role")
                else User.objects.filter(is_staff=False).count()
            )
        else:
    ...
    def view(self, request):
        ...
        if not os.path.isfile(full_path) or not os.path.abspath(full_path).startswith(
            os.path.abspath(documents_path)
        ):
            return Response(
                {"error": "Fichier non trouvé"}, status=status.HTTP_404_NOT_FOUND
            )
...
class DocumentShareViewSet(viewsets.ModelViewSet):
    ...
    def shared_with_me(self, request):
        ...
        user_shares = DocumentShare.objects.filter(
            shared_with=user, expires_at__isnull=True
        ) | DocumentShare.objects.filter(
            shared_with=user, expires_at__gt=timezone.now()
        )
        ...
        folder_shares = DocumentShare.objects.filter(
            shared_with_folder_id__in=user_folders, expires_at__isnull=True
        ) | DocumentShare.objects.filter(
            shared_with_folder_id__in=user_folders, expires_at__gt=timezone.now()
        )
        ...
        return Response({"count": all_shares.count(), "results": serializer.data})
    ...
    def shared_by_me(self, request):
        ...
        shares = DocumentShare.objects.filter(shared_by=request.user).order_by(
            "-shared_at"
        )

        serializer = DocumentShareSerializer(shares, many=True)
        return Response({"count": shares.count(), "results": serializer.data})
    ...
    def mark_accessed(self, request, pk=None):
        ...
        if share.shared_with_folder:
            if (
                (user.branch and user.branch.id == share.shared_with_folder.id)
                or (
                    user.department
                    and user.department.id == share.shared_with_folder.id
                )
                or (user.pole and user.pole.id == share.shared_with_folder.id)
                or (
                    user.branch
                    and user.branch.parent
                    and user.branch.parent.id == share.shared_with_folder.id
                )
                or (
                    user.department
                    and user.department.parent
                    and user.department.parent.id == share.shared_with_folder.id
                )
                or (
                    user.department
                    and user.department.parent
                    and user.department.parent.parent
                    and user.department.parent.parent.id == share.shared_with_folder.id
                )
            ):
                is_recipient = True
