...
def create_department_folders_on_upload(sender, instance, created, **kwargs):
    ...
    try:
        ...
        if not agent_branch:
            logger.error(f"❌ CRITIQUE: Agent {instance.agent.matricule} sans branche assignée!")
            logger.error(f"❌ Document {instance.id} NON ROUTÉ - Assigner une branche à l'agent d'abord")
            return
        
        # 🔒 VALIDATION CRITIQUE 2: La branche DOIT avoir un dossier racine
        branch_folder = agent_branch.folder
        if not branch_folder:
            logger.error(f"❌ CRITIQUE: Branche {agent_branch.name} sans dossier racine!")
            logger.error(f"❌ Document {instance.id} NON ROUTÉ - Créer le dossier racine de la branche d'abord")
            return
        
        # 🔒 VALIDATION CRITIQUE 3: Le dossier branche DOIT être à la racine (parent=None)
        if branch_folder.parent is not None:
            logger.error(f"❌ CRITIQUE: Dossier branche {branch_folder.id} ({branch_folder.name}) a un parent!")
            logger.error(f"❌ Structure corrompue détectée - parent_id={branch_folder.parent_id}")
            logger.error(f"❌ Document {instance.id} NON ROUTÉ pour éviter corruption")
            return
        
        logger.info(f"✅ Dossier branche valide: {branch_folder.id} ({branch_folder.name}) [parent=None]")
        
        # ==================== ÉTAPE 2: CRÉATION DOSSIER DÉPARTEMENT ====================
        
        logger.info(f"🔍 Recherche/création dossier département: '{agent_department.name}' sous branche {branch_folder.id}")
        ...
        if created_dept:
            logger.info(f"✅ Dossier département CRÉÉ: {dept_folder.id} ({dept_folder.name})")
        else:
            logger.info(f"✅ Dossier département existant: {dept_folder.id} ({dept_folder.name})")
        
        # 🔒 VALIDATION POST-CRÉATION: Vérifier que le dossier département a bien le bon parent
        if dept_folder.parent != branch_folder:
            logger.warning(f"⚠️ CORRECTION: Dossier département avait parent={dept_folder.parent_id}, correction à {branch_folder.id}")
        ...
        if agent_department.folder != dept_folder:
            ...
            logger.info(f"✅ Département {agent_department.name} lié au dossier {dept_folder.id}")
        ...
        if not dept_folder.parent:
            ...
            logger.error(f"❌ Ne pas créer de sous-dossier pour éviter orphelins à la racine")
            return
        
        logger.info(f"🔍 Création sous-dossier '{subfolder_name}' sous département {dept_folder.id}")
        ...
        if created_sub:
            ...
            logger.info(f"✅ Sous-dossier existant: {sub_folder.id} ({sub_folder.name})")
        
        # 🔒 VALIDATION POST-CRÉATION: Vérifier que le sous-dossier a bien le bon parent
        if sub_folder.parent != dept_folder:
            logger.warning(f"⚠️ CORRECTION: Sous-dossier avait parent={sub_folder.parent_id}, correction à {dept_folder.id}")
        ...
        logger.error(f"❌ ERREUR lors du traitement du document {instance.id}: {str(e)}", exc_info=True)
...
def auto_route_document(sender, instance, created, **kwargs):
    ...
    if not (hasattr(instance, 'validation_result') and 
            instance.validation_result and 
            instance.validation_result.status == 'PASSED' and 
            instance.status == 'EN_ATTENTE' and 
            not instance.folder):
        return
    ...
    try:
        ...
        rules = RoutingRule.objects.filter(rules_query, is_active=True).order_by('-priority', '-created_at')
        
        logger.info(f"🔍 Recherche règles de routage pour document {instance.id}")
        if agent_pole:
            logger.info(f"   Pôle: {agent_pole.name} | Filiale: {agent_branch.name if agent_branch else 'None'}")
        ...
        for rule in rules:
            if rule.matches(instance):
                ...
                if rule.apply_routing(instance):
                    logger.info(f"✅ Document {instance.id} routé par règle '{rule.name}'")
                    break
                else:
        ...
        logger.error(f"❌ Erreur lors du routage automatique du document {instance.id}: {str(e)}", exc_info=True)
...
def archive_rejected_documents(sender, instance, created, **kwargs):
    ...
    try:
        ...
        agent_branch = instance.agent.branch
        logger.info(f"🗄️ Archivage document {instance.id} rejeté pour branche {agent_branch.name}")
        ...
        if not archive_folder:
            logger.warning(f"⚠️ Dossier Archive introuvable pour branche {agent_branch.name}")
            return
        
        # Archiver le document
        instance.folder = archive_folder
        instance.save(update_fields=['folder'])
        logger.info(f"✅ Document {instance.id} archivé dans {archive_folder.get_full_path()}")
        
    except Exception as e:
        logger.error(f"❌ Erreur lors de l'archivage du document {instance.id}: {str(e)}", exc_info=True)
...
def notify_validation_result(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.error(f"❌ Erreur lors de la notification de validation: {str(e)}", exc_info=True)
...
def create_folders_on_document_type_added(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.error(f"❌ Erreur lors de la création du sous-dossier: {str(e)}", exc_info=True)
