...
class FileTypeConfiguration(models.Model):
    ...
    display_name = models.CharField(
        max_length=100, help_text="Nom d'affichage du type de fichier"
    )
    ...
    max_width_px = models.PositiveIntegerField(
        null=True, blank=True, help_text="Largeur maximale en pixels (pour les images)"
    )
    max_height_px = models.PositiveIntegerField(
        null=True, blank=True, help_text="Hauteur maximale en pixels (pour les images)"
    )
    min_width_px = models.PositiveIntegerField(
        null=True, blank=True, help_text="Largeur minimale en pixels (pour les images)"
    )
    min_height_px = models.PositiveIntegerField(
        null=True, blank=True, help_text="Hauteur minimale en pixels (pour les images)"
    )
    ...
    require_no_password = models.BooleanField(
        default=False, help_text="Le fichier ne doit pas être protégé par mot de passe"
    )
    ...
    forbidden_columns = models.JSONField(
        default=list, blank=True, help_text="Colonnes interdites par nom ou index"
    )
    required_columns = models.JSONField(
        default=list, blank=True, help_text="Colonnes obligatoires"
    )
    ...
    is_auto_validated = models.BooleanField(
        default=True, help_text="Valider automatiquement lors de l'upload"
    )
