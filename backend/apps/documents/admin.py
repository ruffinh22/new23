...
class DocumentSpecificationAdmin(admin.ModelAdmin):
    ...
    def formfield_for_dbfield(self, db_field, request, **kwargs):
        ...
        if db_field.name == "allowed_formats":
            ...
            formfield.help_text = (
                "Nom exact de la feuille Excel (laissez vide pour première feuille)"
            )
...
class DocumentValidationResultAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Document", {"fields": ("document", "validated_at")}),
        ("Résultat de validation", {"fields": ("status",)}),
        (
            "Erreurs",
            {
                "fields": ("errors",),
...
class DocumentAdmin(admin.ModelAdmin):
    ...
    def agent_with_dept(self, obj):
        ...
        if obj.agent:
            return format_html(
                ...
                f"Dept: {obj.agent.department}"
                if obj.agent.department
                else "Dept: N/A",
            )
        return "—"
    ...
    def mark_as_validated(self, request, queryset):
        ...
        self.message_user(
            request, f"✅ {updated} document(s) marqué(s) comme validé(s)."
        )
    ...
    def mark_as_rejected(self, request, queryset):
        ...
        self.message_user(
            request, f"❌ {updated} document(s) marqué(s) comme rejeté(s)."
        )
    ...
    def export_as_csv(self, request, queryset):
        ...
        self.message_user(
            request, f"📊 {queryset.count()} document(s) exporté(s) en CSV."
        )
        return response
    ...
    def get_queryset(self, request):
        ...
        return qs.select_related(
            "agent", "folder", "specification", "validation_result"
        ).prefetch_related()
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if request.user.is_superuser and obj:
            ...
            return [
                ...
                if f not in ["status", "rejection_reason"]
            ]
    ...
    class Media:
        ...
        css = {"all": ("admin/documents_admin.css",)}
