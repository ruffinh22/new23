...
class DocumentListSerializer(serializers.ModelSerializer):
    ...
    agent_department = serializers.CharField(
        source="agent.department.name", read_only=True, allow_null=True
    )
...
class DocumentDetailSerializer(serializers.ModelSerializer):
    ...
    folder_id = serializers.IntegerField(
        source="folder.id", read_only=True, allow_null=True
    )
    ...
    routing_rule_name = serializers.CharField(
        source="routing_rule_applied.name", read_only=True, allow_null=True
    )
...
class DocumentCreateSerializer(serializers.ModelSerializer):
    ...
    document_type = serializers.IntegerField(
        required=True, help_text="ID du type de document"
    )

    # Champs pour la distribution du document au destinataire
    send_to_recipient = serializers.BooleanField(
    ...
    recipient_pole_id = serializers.IntegerField(
        required=False, allow_null=True, write_only=True
    )
    recipient_filiale_id = serializers.IntegerField(
        required=False, allow_null=True, write_only=True
    )
    recipient_service_id = serializers.IntegerField(
        required=False, allow_null=True, write_only=True
    )
    ...
    def validate(self, data):
        ...
        if document_type_id:
            try:
                ...
                document_type_obj = DocumentType.objects.get(
                    id=document_type_id, is_active=True
                )
        ...
        logger.info(f"""
        🔍 [VALIDATE] Données reçues du formulaire:
    ...
    def validate_file(self, value):
        ...
        if not is_valid:
            raise ValidationError(
                error_message
                or "Fichier invalide selon la configuration des types de fichiers"
            )
    ...
    def create(self, validated_data):
        ...
        folder_id = validated_data.pop(
            "folder_id", None
        )  # Extraire folder_id du frontend
        ...
        logger = logging.getLogger(__name__)
        logger.info(f"""
        📋 [DOCUMENT CREATE] Début création document
        - send_to_recipient: {send_to_recipient} (type: {type(send_to_recipient).__name__})
        - recipient_type: {recipient_type}
        - recipient_user_id: {recipient_user_id}
        - folder_id (du frontend): {folder_id}
        ...
        with transaction.atomic():
            ...
            if send_to_recipient and recipient_type == "user" and recipient_user_id:
                logger.info(
                    f"✓ Condition send_to_recipient OK - créer dossier Received pour user {recipient_user_id}"
                )
                recipient_user = User.objects.get(id=recipient_user_id)
                ...
                logger.info(
                    f"✓ Dossier Received créé/trouvé: {received_folder.name} (ID: {received_folder.id}), created={created}"
                )
            ...
            - Folder: {document.folder.name} (ID: {document.folder.id})
...
class FileTypeRequirementSerializer(serializers.Serializer):
    ...
    routing_rule_name = serializers.CharField(
        source="routing_rule.name", read_only=True
    )
    file_type_config_id = serializers.IntegerField()
    file_type_config = FileTypeConfigurationSerializer(
        source="file_type_config", read_only=True
    )
...
class DocumentTransferSerializer(serializers.ModelSerializer):
    ...
    from_folder_name = serializers.CharField(
        source="from_folder.name", read_only=True, allow_null=True
    )
    to_folder_name = serializers.CharField(
        source="to_folder.name", read_only=True, allow_null=True
    )
    transferred_by_name = serializers.CharField(
        source="transferred_by.get_full_name", read_only=True, allow_null=True
    )
