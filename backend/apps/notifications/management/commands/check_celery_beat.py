...
class Command(BaseCommand):
    help = (
        "Vérifier et corriger les problèmes de Celery Beat (timezone, timestamps, etc.)"
    )
        ...
        for task in tasks:
            ...
            if task.start_time:
                if timezone.is_naive(task.start_time):
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ⚠️  {task.name}: Datetime naive (sans timezone)"
                        )
                    )
                    if fix:
                        # Convertir en aware
            ...
            if task.task:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'  ❌ {task.name}: Tâche "{task.task}" non trouvée'
                        )
                    )
                    issues += 1
