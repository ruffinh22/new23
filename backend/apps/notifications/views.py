...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Notification.objects.filter(recipient=user, is_archived=False).order_by(
            "-created_at"
        )
    ...
    def unread_count(self, request):
        ...
        return Response({"count": count, "timestamp": timezone.now().isoformat()})
    ...
    def mark_as_read(self, request, pk=None):
        ...
        if notification.recipient != request.user and not request.user.is_staff:
            log_error(
                "Permission denied for mark_as_read",
                logger_instance=notifications_logger,
                user_id=request.user.id,
                notification_id=pk,
            )
    ...
    def bulk_archive(self, request):
        ...
        try:
            user = request.user
            count = Notification.objects.filter(
                recipient=user, is_archived=False
        ...
        try:
            ...
            if request.method == "GET":
                ...
                if serializer.is_valid():
                    ...
                    return Response(
                        {"detail": "Préférences mises à jour", "data": serializer.data},
            ...
            log_error(
                "Preferences error", exception=e, logger_instance=notifications_logger
            )
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def statistics(self, request):
        ...
        try:
            ...
            log_error(
                "Statistics error", exception=e, logger_instance=notifications_logger
            )
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
