...
def send_notification_email_async(self, notification_id, retry_count=0):
    ...
    try:
        ...
        if not NotificationService.should_send_notification(
            ...
            log_info(
                f'Email skipped (user preferences)',
    ...
    for i in range(0, len(notification_ids), batch_size):
        ...
        for notif_id in chunk:
            try:
                ...
                log_error(f'Batch email error', exception=e, logger_instance=notifications_logger)
                failed_count += 1
    
    log_info(
        f'Batch emails completed',
        notifications_logger,
        sent=sent_count,
        failed=failed_count,
...
def archive_old_notifications():
    ...
    try:
        ...
        log_error('Auto-archive failed', exception=e, logger_instance=notifications_logger)
        return 0
...
def send_daily_digest(user_id):
    ...
    try:
        ...
        message = f"Vous avez {unread} notifications non lues. Consultez l'app pour les voir."
        ...
        log_error('Daily digest failed', exception=e, logger_instance=notifications_logger)
        return 'failed'
...
L'équipe SGDRA
        ...
        for user in users:
            try:
                ...
                log_error(f"Failed to send email to {user.email}", exception=e, logger_instance=base_logger)
