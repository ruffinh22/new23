...
class EmailSchedule(models.Model):
    ...
    def clean(self):
        ...
        if self.recipient_type != 'ALL_USERS' and not self.recipient_folder:
            raise ValidationError("Un dossier destinataire est requis pour ce type de destinataire")
        
        # Validation pour ONE_TIME
        if self.recurrence_type == 'ONE_TIME' and not self.scheduled_at:
            raise ValidationError("Une date et heure d'envoi est requise pour une programmation unique")
        
        # Validation pour MONTHLY
        if self.recurrence_type == 'MONTHLY':
            if not self.monthly_days:
                raise ValidationError("Au moins un jour du mois doit être sélectionné pour une programmation mensuelle")
            if not self.monthly_time:
                raise ValidationError("Une heure d'envoi est requise pour une programmation mensuelle")
            ...
            try:
                ...
                for day in days:
                    if day < 1 or day > 31:
                        raise ValueError(f"Le jour {day} est invalide (doit être entre 1 et 31)")
    ...
    def get_recipient_emails(self):
        ...
        if self.recipient_type == 'ALL_USERS':
            ...
            if folder_type == 'pole':
                ...
                return list(
                    User.objects.filter(
                        branch__parent=self.recipient_folder,
                        is_active=True
                ...
                return list(
                    User.objects.filter(
                        department=self.recipient_folder,
                        is_active=True
                    ).values_list('email', flat=True)
                )
...
class Event(models.Model):
    ...
    # Lieu (optionnel)
    location = models.CharField(max_length=255, blank=True, help_text="Lieu de l'événement")
