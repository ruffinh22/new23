...
class EmailScheduleViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """Assigne l'utilisateur courant comme créateur et change le statut à SCHEDULED"""
        # Sauvegarder avec created_by ET passer le status à SCHEDULED pour que Celery le traite
        serializer.save(created_by=self.request.user, status="SCHEDULED")
    ...
    def perform_update(self, serializer):
        ...
        if obj.status in ["SENT", "FAILED"]:
            ...
            raise PermissionDenied(detail="Impossible de modifier un email déjà envoyé")
        serializer.save()
...
class EventViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "POLE_MANAGER" and user.pole:
            return (
                Event.objects.filter(
                    ...
                    | Q(
                        status="PUBLISHED", is_public=False, visible_to_folder=user.pole
                    )  # Événements du pôle
                )
                .distinct()
                .order_by("-event_date")
            ...
            return (
                Event.objects.filter(
                    ...
                    | Q(
                        status="PUBLISHED",
                        is_public=False,
                        visible_to_folder=user.branch,
                    )  # Événements de la filiale
                )
                .distinct()
                .order_by("-event_date")
            )
            ...
            return (
                Event.objects.filter(
                    ...
                    | Q(
                        status="PUBLISHED",
                        is_public=False,
                        visible_to_folder=user.department,
        ...
        return Event.objects.filter(status="PUBLISHED", is_public=True).order_by(
            "-event_date"
        )
    ...
    def upcoming(self, request):
        ...
        upcoming_events = self.get_queryset().filter(event_date__gte=timezone.now())[
            :10
        ]
