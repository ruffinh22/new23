...
class BranchViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        return (
            self.get_action_permissions(self.action)
            if hasattr(self, "get_action_permissions")
            else [
    ...
    def choices(self, request):
        ...
        choices = Folder.objects.filter(folder_type="filiale", is_active=True).values(
            "id", "name"
        )
...
class DepartmentViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def choices(self, request):
        ...
        choices = Folder.objects.filter(folder_type="service", is_active=True).values(
            "id", "name"
        )
        return Response(list(choices))
