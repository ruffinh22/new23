...
def log_error(message: str, exception=None, logger_instance=None, **kwargs):
    ...
    lg = logger_instance or logger
    if exception:
        lg.error(message, exc_info=True, extra={'context': kwargs, 'exception': str(exception)} if kwargs else {'exception': str(exception)})
    else:
...
def log_audit(action: str, user_id=None, resource_type=None, resource_id=None, changes=None, **kwargs):
...
def log_database_operation(operation: str, model_name: str, count=None, duration_ms=None):
...
def log_api_request(method: str, path: str, user_id=None, status_code=None, duration_ms=None):
    """Log API request."""
...
def log_external_call(service_name: str, endpoint: str, status_code=None, duration_ms=None, error=None):
