...
def safe_format_suffix_patterns(urlpatterns, suffix_pattern=None, default=None):
    ...
    try:
        ...
        logger.debug(
            f"format_suffix_patterns applied successfully (call #{_format_suffix_applied_count})"
        )
        return result
        ...
        if "already registered" in str(e):
            ...
            logger.debug(
                f"DRF format_suffix converter already registered (call #{_format_suffix_applied_count + 1}), skipping"
            )
            return urlpatterns
        else:
