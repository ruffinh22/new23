...
class RequestValidator:
    ...
    @staticmethod
    def validate_range(
        value: int, min_val: int = None, max_val: int = None, field_name: str = "value"
    ) -> None:
        ...
        if min_val is not None and value < min_val:
            raise DRFValidationError({field_name: f"Value must be >= {min_val}"})

        if max_val is not None and value > max_val:
            raise DRFValidationError({field_name: f"Value must be <= {max_val}"})

    @staticmethod
    def validate_length(
        text: str, min_len: int = None, max_len: int = None, field_name: str = "text"
    ) -> None:
...
class DocumentValidator(RequestValidator):
    ...
    def validate_document_create(cls, data: Dict) -> None:
        ...
        # Longueur
        cls.validate_length(
            data.get("title"), min_len=3, max_len=255, field_name="title"
        )
    ...
    def validate_document_update(cls, data: Dict, partial: bool = True) -> None:
        ...
        if "title" in data:
            ...
            cls.validate_length(
                data["title"], min_len=3, max_len=255, field_name="title"
            )

        # Document type si fourni
        if "document_type" in data:
            cls.validate_choice(
                data["document_type"], cls.VALID_DOCUMENT_TYPES, "document_type"
            )
...
class FolderValidator(RequestValidator):
    ...
    def validate_folder_create(cls, data: Dict) -> None:
        ...
        # Énuméracion
        cls.validate_choice(
            data.get("folder_type"), cls.VALID_FOLDER_TYPES, "folder_type"
        )
