...
class AuditMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        if response.status_code == 403:
            user_info = f"{user.matricule} ({user.first_name} {user.last_name})" if user and user.is_authenticated else "Anonymous"
            logger.warning(
    ...
    def process_exception(self, request, exception):
        """Log les exceptions."""
        user = request.user if hasattr(request, 'user') else None
        user_info = f"{user.matricule}" if user and hasattr(user, 'matricule') else "Unknown"
