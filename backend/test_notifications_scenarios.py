...
def get_users():
    ...
    other_admin = (
        User.objects.filter(is_staff=True, is_active=True).exclude(id=admin.id).first()
    )
...
def test_scenario_3():
    ...
    for notif in new_notifs:
        ...
        print(
            f"  ✉️  {recipient_type}: {notif.recipient.matricule} ({notif.recipient.first_name})"
        )
        print(f"     Titre: {notif.title}")
...
def test_scenario_4():
    ...
    for notif in new_notifs:
        ...
        if "raison" in notif.message.lower() or "Raison" in notif.message:
            print("     ✓ Raison incluse dans le message")
