...
def test_document_upload_path():
    ...
    mappings = DepartmentDocumentType.objects.filter(department="RH", is_available=True)
    print(f"Mappings RH disponibles: {mappings.count()}")
    for m in mappings[:3]:
        print(
            f"  - {m.get_document_type_display()}: {m.target_folder.name if m.target_folder else 'Pas de dossier'}"
        )
    ...
    print(f"\n✅ Chemin généré: {path}")
    print("   Structure: documents/DEPT/TYPE_FOLDER/fichier")
    ...
    departments = DepartmentDocumentType.objects.values_list(
