...
class E2EWorkflowTestCase(APITestCase):
    ...
    def _get_token(self, matricule, password):
        ...
        response = self.client.post(
            "/api/users/login/", {"matricule": matricule, "password": password}
        )
    ...
    def test_complete_workflow_upload_to_approval(self):
        ...
        response = self.client.post(
            ...
            {"validation_notes": "Données valides et complètes", "is_valid": True},
        )
        ...
        response = self.client.post(
            ...
            {"approval_notes": "Approuvé pour traitement", "approved": True},
        )
    ...
    def test_concurrent_document_modification(self):
        ...
        # Valider le document
        self._authenticate("VAL001")
        self.client.post(f"/api/documents/{doc_id}/validate/", {"is_valid": True})

        # Approver 1: Approuve le document
        print("  1️⃣  Approbateur 1 approuve...")
        self._authenticate("APP001")
        response1 = self.client.post(
            f"/api/documents/{doc_id}/approve/", {"approved": True}
        )
    ...
    def test_notification_delivery_timing(self):
        ...
        response = self.client.post(
            f"/api/documents/{doc_id}/validate/", {"is_valid": True}
        )
    ...
    def test_search_and_filter_performance(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        print(
            f"     ✅ Listing en {elapsed:.3f}s ({len(response.data.get('results', []))} docs)"
        )
    ...
    def test_error_handling_and_recovery(self):
        ...
        response = self.client.post(
            "/api/documents/",
            {"document_type": "DONNEES_AGENTS", "description": "Sans fichier"},
            format="multipart",
        )
        ...
        response = self.client.post(
            "/api/documents/",
            {"document_type": "INVALID_TYPE", "uploaded_file": file},
            format="multipart",
        )
...
class E2EPerformanceTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.agent = AgentUser.objects.create_user(
            matricule="PERF001", email="perf@example.com", password="test123456"
        )

    def _authenticate(self):
        """Authentifier"""
        token = self.client.post(
            "/api/users/login/", {"matricule": "PERF001", "password": "test123456"}
        ).data["access"]
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
