...
def setup_smart_mappings():
    ...
    all_mappings = DepartmentDocumentType.objects.filter(is_available=True).order_by(
        "department"
    )
    ...
    for mapping in all_mappings:
        ...
        if folder:
            ...
            mapping.save()
            print(
                f"✅ {mapping.department:12} + {mapping.get_document_type_display():15} → {folder.name}"
            )
            count += 1
        else:
            print(
                f"⚠️  {mapping.department:12} + {mapping.get_document_type_display():15} → [Aucun dossier trouvé]"
            )
