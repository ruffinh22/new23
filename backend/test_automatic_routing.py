...
def test_routing_system():
    ...
    mappings = DepartmentDocumentType.objects.filter(is_available=True).select_related('target_folder')
    ...
    for mapping in mappings[:10]:  # Afficher les 10 premiers
        folder_name = mapping.target_folder.name if mapping.target_folder else "❌ PAS DE DOSSIER"
        print(f"   • {mapping.department:12} + {mapping.document_type:20} → {folder_name}")
    ...
    try:
        ...
        if created:
            print(f"✅ Agent test créé: {agent.username} (matricule: {agent.matricule}, dept: {agent.department})")
        else:
            print(f"✅ Agent test trouvé: {agent.username} (matricule: {agent.matricule}, dept: {agent.department})")
        ...
        for doc_type, display_name in test_cases:
            ...
            print(f"     - département: {parts[1]} {'✅' if parts[1] == 'RH' else '❌'}")
    ...
    if mapping_with_folder:
        ...
        if 'target_folder' in data and 'target_folder_id' in data:
            print(f"   ✅ Les champs target_folder et target_folder_id sont exposés")
        else:
            print(f"   ❌ Les champs target_folder/target_folder_id ne sont pas exposés")
    else:
    ...
    print(f"   • Sérialiseur exposant target_folder: OK ✅")
    print("\n💡 Prochaines étapes:")
